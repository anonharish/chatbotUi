  ...
  const selectedDistrictsData = useMemo(() => {
    return Array.from(currentSelection).map((key) => {
      ...
      return (
        info || {
          id: key,
          featureId: parsed?.featureId || 0,
          name: `District ${parsed?.featureId}`,
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          background:
            "radial-gradient(circle at 50% 50%, rgba(56, 189, 248, 0.15) 0%, rgba(56, 189, 248, 0.05) 30%, transparent 60%)",
        }}
      />
        ...
        <div className="absolute bottom-6 left-6 z-[1000]">
          <div className="flex items-center gap-2 bg-cyan-900/80 text-cyan-100 px-4 py-2 rounded-lg border border-cyan-500/50 shadow-lg">
            ...
            <span className="font-bold">
              {currentSelection.size} district
              {currentSelection.size !== 1 ? "s" : ""} selected
            </span>
          </div>
        </div>
      )}
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between px-1">
                        <span className="text-[10px] uppercase tracking-wider text-white/40 font-bold">
                          Selected Districts ({selectedDistrictsData.length})
                        </span>
                      </div>
                      <div className="flex flex-wrap gap-2 p-3 bg-white/5 border border-white/10 rounded-xl max-h-40 overflow-y-auto custom-scrollbar">
