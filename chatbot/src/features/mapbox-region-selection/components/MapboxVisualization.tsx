...
// Colors
const DISTRICT_HOVER_COLOR = "rgba(255, 191, 0, 0.4)";
const DISTRICT_SELECTED_COLOR = "rgba(0, 191, 255, 0.85)";
...
export const MapboxVisualization = ({
  ...
  // Keep callbacks in refs to avoid stale closures inside map event handlers
  ...
  // Only zoom + load districts when the user explicitly clicks a state.
  // selectedState prop change → zoom in. null → fly back to India.
  useEffect(() => {
  ...
  // Update district feature-state colours when regions or selection change
  useEffect(() => {
    ...
    for (let featureId = 0; featureId < count; featureId++) {
      ...
      const region = regions.find((r) => r.districtIds.has(districtKey || ""));
      const isSelected = districtKey ? currentSelection.has(districtKey) : false;

      map.setFeatureState(
        { source: "state-districts", id: featureId },
        { regionColor: region?.color || null, selected: isSelected },
      );
    }
  }, [regions, currentSelection]);

  // ── Keep the persistent all-regions polygon layer in sync ───────────────────
  // This renders saved region polygons at ANY zoom level (including global view).
  // The layer uses the geometry stored in each Region object, so it works
  // immediately after a refresh without needing the user to zoom into a state.
  useEffect(() => {
    const map = mapRef.current;
    if (!map) return;

    if (!map.isStyleLoaded()) {
      map.once("styledata", () => syncAllRegionsLayer(map));
      return;
    }

    syncAllRegionsLayer(map);
  }, [regions]);

  function syncAllRegionsLayer(map: maplibregl.Map) {
    ...
    if (existingSource) {
      // Just update data — layers + handlers already registered
    ...
    // First time: create source, layers, and event handlers
    map.addSource("all-regions", {
      type: "geojson",
      data: featureCollection,
    });

    // Fill — inserted BELOW india-state-fill so state hover still works on top
    map.addLayer(
      {
        id: "all-regions-fill",
        type: "fill",
    ...
    map.on("mouseleave", "all-regions-fill", () => {
      ...
      map.querySourceFeatures("all-regions").forEach((f) => {
        if (f.id !== undefined) {
          map.setFeatureState({ source: "all-regions", id: f.id }, { hover: false });
        }
      });
    ...
    map.on("mousemove", "all-regions-fill", (e) => {
      ...
      map.setFeatureState({ source: "all-regions", id: featureId }, { hover: true });

      const props = feature.properties;
      let loadedOfficers: FieldOfficer[] = [];
      try {
        if (props?.fieldOfficersRaw) loadedOfficers = JSON.parse(props.fieldOfficersRaw);
      } catch (_) {}
  ...
  useEffect(() => {
    ...
    const map = new maplibregl.Map({
      ...
      // Always start at global India view — never restore a zoomed-in position
      center: INDIA_CENTER,
      zoom: INITIAL_ZOOM,
      maxPitch: 85,
    });
    ...
    const zoomToState = (stateName: string) => {
      ...
      map.fitBounds(
        ...
        { padding: { top: 50, bottom: 50, left: 50, right: 50 }, duration: 1500, maxZoom: 8 },
      );
    };
    ...
    const loadStateDistricts = (stateName: string) => {
      ...
      fetch(districtsUrl)
        ...
        .then((data: FeatureCollection) => {
          ["region-fill", "district-borders", "district-fill"].forEach((id) => {
            if (map.getLayer(id)) map.removeLayer(id);
          });
          if (map.getSource("state-districts")) map.removeSource("state-districts");
          ...
          map.addLayer({
            ...
            paint: {
              "fill-color": [
                "case",
                ["boolean", ["feature-state", "selected"], false], DISTRICT_SELECTED_COLOR,
                ["boolean", ["feature-state", "hover"], false], DISTRICT_HOVER_COLOR,
                "rgba(0, 0, 0, 0)",
              ],
              "fill-opacity": [
                "case",
          ...
          map.addLayer({
            ...
            paint: {
              ...
              "fill-opacity": [
                "case",
                ["to-boolean", ["feature-state", "regionColor"]], 0.7,
                0,
              ],
            },
          });
          ...
          for (let featureId = 0; featureId < featuresWithIds.length; featureId++) {
            ...
            const region = regionsRef.current.find((r) => r.districtIds.has(districtKey));
            if (region) {
              map.setFeatureState(
                { source: "state-districts", id: featureId },
                { regionColor: region.color, selected: false },
              );
            }
          }
        })
    ...
    actionsRef.current = {
      ...
      flyToIndia: () => {
        ...
        ["district-fill", "district-borders", "region-fill"].forEach((id) => {
          if (map.getLayer(id)) map.removeLayer(id);
        });
        if (map.getSource("state-districts")) map.removeSource("state-districts");
        districtsLoadedRef.current = false;
      ...
      fetch(INDIA_STATES_URL)
        ...
        .then((data: FeatureCollection) => {
          ...
          try {
            ...
            if (indiaOutline) {
              map.addSource("india-outline", { type: "geojson", data: indiaOutline });
              map.addLayer({
              ...
              map.addLayer({
                ...
                type: "line",
                source: "india-outline",
                paint: { "line-color": "#ffffff", "line-width": 2, "line-opacity": 0.5, "line-blur": 2 },
              });
            }
          } catch (e) {
          ...
          // ── Render saved regions immediately after state layers are ready ──
          // This ensures colored polygons appear on the global map right after
          // a refresh, before the user zooms into any state.
          syncAllRegionsLayer(map);
        });
    });
    ...
    map.on("click", "india-state-fill", (e) => {
      ...
      if (map.getLayer("district-fill")) {
        const hits = map.queryRenderedFeatures(e.point, { layers: ["district-fill"] });
    ...
    map.on("mousemove", "district-fill", (e) => {
      ...
      map.setFeatureState({ source: "state-districts", id: featureId }, { hover: true });
              ...
              <div className="text-xs space-y-1 pt-2 border-t border-slate-700/50">
                ...
                {tooltip.regionInfo.regionalOfficer && (
                  <div className="text-slate-400">
                    RO: <span className="text-slate-200">{tooltip.regionInfo.regionalOfficer}</span>
                  </div>
                )}
                {tooltip.regionInfo.intelligentOfficer && (
                  <div className="text-slate-400">
                    IO: <span className="text-slate-200">{tooltip.regionInfo.intelligentOfficer}</span>
                  </div>
                )}
              </div>
                    ...
                    <div className="space-y-1 mt-1 mb-2">
                      {tooltip.regionInfo.fieldOfficers.slice(0, 3).map((fo) => (
                        <div key={fo.id} className="flex items-center gap-1.5 text-xs text-slate-300">
                          <div className="w-1.5 h-1.5 rounded-full" style={{ background: fo.color }} />
